...
- Synchronous success means path lines in text mode, or `ok: true` plus non-empty `images[].path` in JSON mode. JSON image entries may use `status=done`; do not require per-image `status=completed`.
